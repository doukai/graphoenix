...
public class JavaFileBuilder {
    ...
    public List<JavaFile> buildJavaFileList() {
        ...
        javaFileList.addAll(typeSpecBuilder.buildObjectTypeExpressionAnnotations().stream().map(typeSpec -> JavaFile.builder(configuration.getBasePackageName(), typeSpec).build()).collect(Collectors.toList()));
        javaFileList.addAll(typeSpecBuilder.buildObjectTypeExpressionsAnnotations().stream().map(typeSpec -> JavaFile.builder(configuration.getBasePackageName(), typeSpec).build()).collect(Collectors.toList()));
